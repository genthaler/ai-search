...
alphabeta depth alpha beta maximizingPlayer isTerminal heuristic getChildren node =
    if depth == 0 then
        heuristic node

    else if maximizingPlayer then
        case List.maximum <| List.map (alphabeta (depth + -1) alpha beta (not maximizingPlayer) isTerminal heuristic getChildren) (getChildren node) of
            Nothing ->
                heuristic node
        ...
        case List.minimum <| List.map (alphabeta (depth + -1) alpha beta (not maximizingPlayer) isTerminal heuristic getChildren) (getChildren node) of
            Nothing ->
                heuristic node
